...
const enumArray = (values: readonly string[], type: string) =>
  `array[${values.map(lit).join(',')}]::${type}[]`;
  ...
  lines.push(
    `insert into ingredients (slug, name, aliases, category, default_unit, grams_per_unit, package_size, package_label, is_staple, is_perishable, indivisible, allergens, data, updated_at) values (` +
      [
        ...
        lit(i.name),
        lit(i.aliases),
        `${lit(i.category)}::shop_category`,
        `${lit(i.default_unit)}::unit_code`,
        lit(i.grams_per_unit ?? null),
        lit(i.package_size ?? null),
        lit(i.package_label ?? null),
        lit(i.is_staple),
        lit(i.is_perishable),
        lit(i.indivisible),
        enumArray(i.allergens, 'allergen'),
        json(i),
        'now()',
      ].join(', ') +
  ...
  lines.push(
    `insert into recipes (slug, name, servings_base, time_min, active_time_min, meal_types, tags, styles, allergens, cost_level, protein_group, batch_reuse, data, updated_at) values (` +
      [
        ...
        lit(r.servings_base),
        lit(r.time_min),
        lit(r.active_time_min ?? null),
        enumArray(r.meal_types, 'meal_type'),
        lit(r.tags),
        enumArray(r.styles, 'diet_style'),
        enumArray(r.allergens, 'allergen'),
        `${lit(r.cost_level)}::cost_level`,
        lit(r.protein_group),
        lit(r.batch_reuse),
        json(r),
        'now()',
...
console.log(
  `✓ ${catalog.ingredients.size} ingredientes y ${catalog.recipes.size} recetas → supabase/seed/catalog.sql (${lines.length} sentencias)`,
);
