...
async function initDataFor(userId: number): Promise<string> {
  const params = new URLSearchParams({
    user: JSON.stringify({ id: userId, first_name: 'Test', language_code: 'es' }),
...
Deno.test('health y catálogo son públicos', async () => {
  const { call } = makeApp();
  assertEquals((await call('GET', '/health')).json.data.phase, 4);
...
Deno.test('PUT /me/state guarda y GET lo devuelve; alergias sin confirmar → 422', async () => {
  ...
  const bad = await call('PUT', '/me/state', a, {
    preferences: { ...PREFS, allergens: ['huevos'], allergens_confirmed: false },
  });
  assertEquals(bad.status, 422);
  const put = await call('PUT', '/me/state', a, {
    preferences: PREFS,
    pantry: ['arroz'],
    favorites: ['c1'],
  });
...
Deno.test('generar menú en el servidor produce 21 huecos y lista de compra', async () => {
  ...
  assertEquals(
    res.json.data.menu.slots.filter((s: { recipe_slug: string | null }) => s.recipe_slug).length,
    21,
  );
  ...
  assertEquals(
    again.json.data.menu.slots.map((s: { recipe_slug: string }) => s.recipe_slug),
    res.json.data.menu.slots.map((s: { recipe_slug: string }) => s.recipe_slug),
  );
});
...
Deno.test('DELETE /me borra el estado; eventos se registran; cuerpo inválido → 422', async () => {
  ...
  await call('PUT', '/me/state', a, { preferences: PREFS });
  assertEquals(
    (await call('POST', '/me/events', a, { screen: 'menu', action: 'generate' })).status,
    204,
  );
