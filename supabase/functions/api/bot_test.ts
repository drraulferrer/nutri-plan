...
const ROWS = {
  ingredients: [ing('tomate', { default_unit: 'ud', grams_per_unit: 150 }), ing('arroz', { category: 'despensa', package_size: 1000, package_label: 'paquete de 1 kg' }), ing('huevos', { default_unit: 'ud', allergens: ['huevos'] }), ing('pepino', { default_unit: 'ud' })],
  recipes: [
    ...
    recipe('con-huevo', ['cena'], { protein_group: 'huevo', allergens: ['huevos'], styles: ['mediterraneo', 'vegetariano'], ingredients: [{ ingredient: 'huevos', quantity: 2, unit: 'ud' }] }),
    recipe('con-pepino', ['cena'], { ingredients: [{ ingredient: 'pepino', quantity: 1, unit: 'ud' }] }),
  ],
};
const PREFS = { people: 2, days: 7, include_snacks: false, cook_time: '30', budget: 'medio', styles: ['mediterraneo'], allergens: ['huevos'], allergens_confirmed: true, disliked_ingredients: [] };
...
async function setup() {
  ...
  const app = createApp(store, { botToken: TOKEN, allowedOrigins: [], botSecret: SECRET, now: () => NOW });
  ...
  const bot = async (method: string, path: string, body?: unknown, secret = SECRET) => {
    const res = await app.request(`http://x/api${path}`, { method, headers: { 'X-Bot-Secret': secret, 'Content-Type': 'application/json' }, body: body === undefined ? undefined : JSON.stringify(body) });
...
Deno.test('rutas /bot deshabilitadas si no hay secreto configurado', async () => {
  const app = createApp(new MemoryStore(ROWS), { botToken: TOKEN, allowedOrigins: [], botSecret: '' });
...
Deno.test('leer y cambiar un hueco; alérgenos y comida incorrecta se rechazan', async () => {
  ...
  assertEquals((await bot('POST', '/bot/context/77/slots/3/cena', { recipe_slug: 'con-huevo' })).status, 422);
  assertEquals((await bot('POST', '/bot/context/77/slots/3/cena', { recipe_slug: 'c1' })).status, 422); // c1 es de comida
  assertEquals((await bot('POST', '/bot/context/77/slots/3/merienda', { recipe_slug: 'n1' })).status, 422);
